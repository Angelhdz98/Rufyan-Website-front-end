...
import { deleteUserAddressdResquest } from "../pages/UserPanelPage/personalUserRequest";
...
interface AddressSelectorProps {
  ...
  confirmAddress: () => void
  onChangeAddresComponent: React.Dispatch<React.SetStateAction<string>>
  //onAddAddress:() => void;  
...
function AddresSelector(addressSelectorProps: AddressSelectorProps) {
  ...
  const deleteAddressHandler = () => {
    deleteUserAddressdResquest(addressSelectorProps.selectedAddress.id).then(() => {
      alert("La dirección ha sido eliminada exitosamente, reinicie la pagina");

    }).catch((error) => {
      alert("Hubo un error eliminando el domicilio: " + error)
    });
  }

  const changeToAddHandler = () => {
    addressSelectorProps.onChangeAddresComponent("form");
  }

  return <div className="flex flex-col mx-4  gap-2  " >
    <div className="flex justify-between p-1">
      <span className="font-semibold m-1 px-[59px]"> Shipment info</span>
      {addressSelectorProps.userAddresses.length <= 2 ?
        <Button onClick={changeToAddHandler} rounded success>
          <FaPlus /> agregar
        </Button>
        :
    ...
    <div className=" flex justify-between pt-1 ">
      ...
      <Button danger rounded onClick={deleteAddressHandler}>Delete Address</Button>
